...
namespace HttpStatus {
    ...
    static inline std::string GetHttpStatusReason(int code) {
        auto it = statusReasons.find(code);
        return (it != statusReasons.end()) ? it->second : "Unknown HTTP Status Code";
    }

    static inline bool IsInformational(int code) {
        return code >= 100 && code < 200;
    }

    static inline bool IsSuccess(int code) {
        return code >= 200 && code < 300;
    }

    static inline bool IsRedirection(int code) {
        return code >= 300 && code < 400;
    }

    static inline bool IsClientError(int code) {
        return code >= 400 && code < 500;
    }

    static inline bool IsServerError(int code) {
